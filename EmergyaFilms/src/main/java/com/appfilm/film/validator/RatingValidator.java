...
public class RatingValidator {
	 ...
	 * 
	 * @param rtng
	 * @return boolean Método para comprobar si el Rating a insertar cumple los
	...
	public boolean validate(Rating rtng) {
		...
		try {
			if (rtng.getScore() < 1 || rtng.getScore() > 5 || scoreComparator == null || rtng.getUserId() == null
				...
				rtng.setMessageRatingJson(
						"No se ha insertado el rating por no cumplir los requisitos, revise los campos --> "
								+ HttpStatus.BAD_REQUEST.getReasonPhrase());
			...
			rtng.setMessageRatingJson(
					"Error de MySQL , el usuario o la pelicula no existe en BD " + HttpStatus.BAD_REQUEST);
