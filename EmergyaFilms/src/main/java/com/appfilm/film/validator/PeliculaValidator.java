...
public class PeliculaValidator {
	 ...
	 * 
	 * @param peli
	 * @return boolean Método para comprobar si la pelicula a insertar cumple los
	...
	public boolean validate(Pelicula peli) {
		...
		try {
			...
			if (peli.getTitle() == null || peli.getTitle().isEmpty() || peli.getDate() == null
					|| peli.getDate().isEmpty() || anio < 1900 || anio > 2018 || peli.getGenres().length <= 0
					|| peli.getGenres().length > 3) {

				log.info("No se cumplen los requisitos de inserción de Peliculas --> "
						+ HttpStatus.BAD_REQUEST.getReasonPhrase());
				peli.setMessagePeliculaJson(
						"No se ha insertado la pelicula por no cumplir los requisitos,rervise los campos --> "
								+ HttpStatus.BAD_REQUEST.getReasonPhrase());
