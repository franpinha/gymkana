...
public class HotMovieController {
	 ...
	 * 
	 * @param parametros
	 * @return ResponseEntity Método a traves el cual mapeamos con nuestro REST
	 *         client(GET) para consultar la pelicula más o menos votada.Realiza una
	 *         comprobación para obtener a traves de los parámetros del GET para
	 *         saber que tipo de pelicula pide.
	 */
	...
	public ResponseEntity<String> consultaMovie(@RequestParam Map<String, String> parametros) {
		...
		try {
			if (report == null || report.isEmpty() || startDate.isEmpty() || endDate.isEmpty()
					|| startDate.length() != 12 || endDate.length() != 12 || !report.equals("hot")
					|| !!report.equals("cold")) {
